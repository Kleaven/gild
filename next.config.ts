...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      {
        // Baseline security headers on every route. CSP is intentionally
        // deferred — the app uses inline styles + Stripe/Supabase/Sentry/
        // PostHog, so a blind CSP would break runtime; add it with nonces
        // as its own change.
        source: '/:path*',
        headers: [
