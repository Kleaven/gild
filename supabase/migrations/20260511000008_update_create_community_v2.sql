-- Migration: Update create_community RPC for Pricing & Onboarding
-- Updates the signature to accept pricing types, amounts, and onboarding messages.

DROP FUNCTION IF EXISTS public.create_community(text, text, text, boolean, text, integer);
...
BEGIN
  ...
  INSERT INTO public.communities (
    ...
    pricing_type, price_amount, price_currency, platform_fee_percent
  )
  VALUES (
    p_name, p_slug, p_description, v_user_id, p_is_private, p_category, 
    p_theme_hue, p_welcome_message, p_goodbye_message,
    p_pricing_type, p_price_amount, p_price_currency, v_fee_percent
  )
